    ...
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-northern-midnight/95 backdrop-blur-xl shadow-lg shadow-aurora-blue/10 safe-top">
              ...
              {user ? (
                <div className="relative ml-2">
                  <button
                    ...
                    className="group flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-dark-800 to-dark-900 border border-aurora-blue/30 hover:border-aurora-green/50 transition-all duration-300"
                  >
                    ...
                    <svg
                      className={`w-3 h-3 text-gray-400 transition-transform duration-300 ${showUserMenu ? 'rotate-180' : ''}`}
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                ...
                <button
                  ...
                  className="ml-2 px-4 py-1.5 bg-gradient-to-r from-aurora-green via-aurora-blue to-aurora-purple text-white text-sm font-semibold rounded-lg hover:shadow-aurora transition-all duration-300"
                >
                  Sign In
                </button>
              )}
            ...
            <div className="md:hidden flex items-center gap-2">
              {user ? (
                <button
                  ...
                  className="flex items-center space-x-2 px-2 py-1.5 rounded-lg bg-dark-800/80 border border-aurora-blue/30"
                >
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-aurora-green via-aurora-blue to-aurora-purple flex items-center justify-center text-xs font-bold">
                    {profile?.full_name?.charAt(0).toUpperCase() || user.email?.charAt(0).toUpperCase()}
                  </div>
                ...
                <button
                  onClick={() => setShowAuthModal(true)}
                  className="px-3 py-1.5 bg-gradient-to-r from-aurora-green to-aurora-blue text-white text-sm font-semibold rounded-lg"
                >
                  Sign In
                </button>
              )}
              ...
              <button
                onClick={() => setShowMenu(!showMenu)}
                className="p-2 rounded-lg bg-dark-800/80 border border-aurora-blue/30"
                aria-label={showMenu ? 'Close menu' : 'Open menu'}
              >
                <div className="w-5 h-5 flex flex-col justify-center items-center">
                  <span className={`block w-5 h-0.5 bg-gradient-to-r from-aurora-green to-aurora-blue rounded-full transition-all duration-300 ${showMenu ? 'rotate-45 translate-y-1' : '-translate-y-1'}`}></span>
                  <span className={`block w-5 h-0.5 bg-gradient-to-r from-aurora-blue to-aurora-purple rounded-full transition-all duration-300 ${showMenu ? 'opacity-0' : 'opacity-100'}`}></span>
                  <span className={`block w-5 h-0.5 bg-gradient-to-r from-aurora-purple to-aurora-pink rounded-full transition-all duration-300 ${showMenu ? '-rotate-45 -translate-y-1' : 'translate-y-1'}`}></span>
                </div>
              </button>
            </div>
          </div>
        ...
        >
          ...
          <div className={`relative max-w-md mx-auto p-4 transition-all duration-500 ${showMenu ? 'translate-y-0' : '-translate-y-full'}`}>
