 ...
 */
export const YELLOWKNIFE_TIMEZONE = 'America/Yellowknife';
 ...
 */
export function getYellowknifeTime(): Date {
  const now = new Date();
  const ykTimeString = now.toLocaleString('en-US', {
    timeZone: YELLOWKNIFE_TIMEZONE,
  });
 ...
 */
export function formatYellowknifeDate(
  date: Date | string,
 ...
 */
export function formatYellowknifeTime(
  date: Date | string,
...
export function isBusinessHours(
  ...
  const dayName = dayNames[date.getDay()] as keyof typeof businessHours;
...
export function generateTimeSlots(
  ...
  slotDuration: number = intervalMinutes
): TimeSlot[] {
 ...
 */
export function getDaylightHours(date: Date | string): number {
  const dateObj = typeof date === 'string' ? new Date(date) : date;
  ...
  if (daysFromSummer > 182) daysFromSummer = 365 - daysFromSummer;
 ...
 */
export function isAuroraSeason(date: Date | string): boolean {
  const dateObj = typeof date === 'string' ? new Date(date) : date;
 ...
 */
export function sortEventsByDateTime(events: ScheduleEvent[]): ScheduleEvent[] {
